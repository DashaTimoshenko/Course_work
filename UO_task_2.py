...
# def func21(x1, x2):
#     global R
#     res = 100 * (x1 ** 2 - x2) ** 2 + (x1 - 1) ** 2 - R * ln(16 - x1 ** 2 - (x2+4) ** 2) - R * ln(4 - x1 ** 2 - x2 ** 2)
#     return res
#
# def condition21(x):
#     x1 = x[0]
...
#     radius2 = 1
#     centre = [0, -4]
#     if centre[0] - radius1 < x1 < centre[0] + radius1 and centre[1] - radius1 < x2 < centre[1] + radius1 and centre[0] - radius2 > x1 > centre[0] + radius2 and centre[1] - radius2 > x2 > centre[1] + radius2:
#         return True
#     else:
#         return False
...
def func21(x1, x2):
    global R
    res = 100 * (x1 ** 2 - x2) ** 2 + (x1 - 1) ** 2 - R * ln(4 - (x1 ** 2) - ((x2 + 4) ** 2)) - R * ln(
        (x1 ** 2) + ((x2 + 4) ** 2) - 1)
...
def condition21(x):
    x1 = x[0]
    x2 = x[1]
    if (x1 ** 2) + ((x2 + 4) ** 2) < 4 and (x1 ** 2) + ((x2 + 4) ** 2) > 1:
        return True
    else:
        return False
...
# total_f_ev_number2 = 0
# while True:
#     x, y, function_evaluations_number = internal_point_method(func21, point_x3, eps, central_derivatives_1,
#                                                               central_derivatives_2, condition21)
#     res_list3.append(x[-1])
#     res_y_list3.append(y[-1])
#     total_f_ev_number2 += function_evaluations_number
#     if (np.linalg.norm(np.array(res_list3[-1] - res_list3[-2])) / np.linalg.norm(
#             np.array(res_list3[-2])) <= eps and abs(res_y_list3[-1] - res_y_list3[-2]) <= eps) or flag == 1:
#         break
